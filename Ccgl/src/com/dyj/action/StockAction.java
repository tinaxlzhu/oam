...
public class StockAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			JSONArray jsonArray = JsonUtil.formatRsToJsonArray(stockdao
					.stockList(con, pageBean, stock));
	...
	public String export2() throws Exception {
		...
		try {
			...
			Workbook wb = ExcelUtil.fillExcelDataWithTemplate(
					stockdao.stockList3(con, null, null),
					"stockExporTemplate.xls", con);
			ResponseUtil3.export(ServletActionContext.getResponse(), wb,
					"利用模版导出excel.xls");
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public String upload() throws Exception {
		...
		if (hssfSheet != null) {
			for (int rowNum = 1; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
				...
				stock.setGoodid(Integer.parseInt(FormatStringUtil
						.formatString(Conversiongood.conversion(ExcelUtil
								.formatCell(hssfRow.getCell(0))))));
				stock.setGoodnumber(Integer.parseInt(FormatStringUtil
						.formatString(ExcelUtil.formatCell(hssfRow.getCell(1)))));
				stock.setStocknote((FormatStringUtil.formatString(ExcelUtil
						.formatCell(hssfRow.getCell(4)))));
				stock.setOutstockid(Integer.parseInt(FormatStringUtil
						.formatString(Conversiongood.conversion(ExcelUtil
								.formatCell(hssfRow.getCell(2))))));
				stock.setStorageid(Integer.parseInt(FormatStringUtil
						.formatString(Conversiongood.conversion(ExcelUtil
								.formatCell(hssfRow.getCell(3))))));
				Connection con = null;
				try {
					con = dbUtil.getCon();
