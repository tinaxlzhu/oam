...
public class GoodAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			JSONArray jsonArray = JsonUtil.formatRsToJsonArray(gooddao
					.goodList(con, pageBean, good));
	...
	public void getOther() throws IOException {
		Map<String, Object> map = new HashMap<String, Object>();
		Connection con = null;
		try {
			con = dbUtil.getCon();
			ArrayList<Goodcategories> goodcategories = goodcategoriesdao
					.getGoodcategoriesList(con);

			JSONObject result = new JSONObject();

			if (goodcategories != null && goodcategories.size() > 0) {
				result.put("success", "true");
				result.put("goodcategories", goodcategories);
			} else {
	...
	public String upload() throws Exception {
		...
		if (hssfSheet != null) {
			for (int rowNum = 0; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
				...
				good.setGoodname(FormatStringUtil.formatString(ExcelUtil
						.formatCell(hssfRow.getCell(1))));
				// good.setSupplierid(FormatStringUtil.formatString(ExcelUtil.formatCell(hssfRow.getCell(2))));
				good.setGoodcategoriesid(FormatStringUtil
						.formatString(ExcelUtil.formatCell(hssfRow.getCell(3))));
				good.setNote(FormatStringUtil.formatString(ExcelUtil
