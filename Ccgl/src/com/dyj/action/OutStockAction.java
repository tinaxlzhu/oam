...
public class OutStockAction extends ActionSupport implements ServletRequestAware{
	...
	public String execute() throws Exception {
		...
		try {
			...
			JSONArray jsonArray = JsonUtil.formatRsToJsonArray(outstockdao
					.outstockList(con, pageBean, outstock));
	...
	public String export2() throws Exception {
		...
		try {
			...
			Workbook wb = ExcelUtil.fillExcelDataWithTemplate(
					outstockdao.outstockList2(con, null, null),
					"outstockExporTemplate.xls", con);
	...
	public String upload() throws Exception {
		...
		if (hssfSheet != null) {
			for (int rowNum = 1; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
				...
				outstock.setGoodid(Integer.parseInt(FormatStringUtil
						.formatString(Conversiongood.conversion(ExcelUtil
								.formatCell(hssfRow.getCell(0))))));
				outstock.setSaleprice(Integer.parseInt(FormatStringUtil
						.formatString(ExcelUtil.formatCell(hssfRow.getCell(1)))));
				outstock.setOutstockdate(DateUtil.formatString(
						hssfRow.getCell(2).getStringCellValue(), "yyyy-MM-dd"));
				outstock.setOutstocknumber(Integer.parseInt((FormatStringUtil
						.formatString(ExcelUtil.formatCell(hssfRow.getCell(3))))));
				outstock.setOutstocknote((FormatStringUtil
						.formatString(ExcelUtil.formatCell(hssfRow.getCell(4)))));
				Connection con = null;
				try {
					con = dbUtil.getCon();
