...
public class StorageDao {
	public ResultSet storageList(Connection con, PageBean pageBean,
			Storage storage) throws Exception {
		StringBuffer sb = new StringBuffer(
				"select t_storage.id,goodid,inprice,storagedate,storagenumber,storagenote,goodname,t_good.id as Goodid,storageunit from t_storage,t_good where t_storage.goodid=t_good.id");
	...
	public ResultSet storageList2(Connection con, PageBean pageBean,
			Storage storage) throws Exception {
		StringBuffer sb = new StringBuffer(
				"select goodname,goodid,inprice,storagedate,storagenumber,storagenote,storageunit from t_storage,t_good where t_storage.goodid=t_good.id");
		if (storage != null && StringUtil.isNotEmpty(storage.getGoodname())) {
			sb.append(" and goodname like '%" + storage.getGoodname() + "%'");
		}
		if (pageBean != null) {
	...
	public ResultSet storageList3(Connection con, PageBean pageBean,
			Storage storage) throws Exception {
		StringBuffer sb = new StringBuffer(
				"select goodname,goodid from t_storage,t_good where t_storage.goodid=t_good.id");
		if (storage != null && StringUtil.isNotEmpty(storage.getGoodname())) {
			sb.append(" and goodname like '%" + storage.getGoodname() + "%'");
		}
		if (pageBean != null) {
	...
	public ResultSet storageList4(Connection con, Storage storage)
			throws Exception {
	...
	public int storageCount(Connection con, Storage storage) throws Exception {
		StringBuffer sb = new StringBuffer(
				"select count(*) as total from t_storage,t_good where t_storage.goodid=t_good.id");
	...
	public int storageAdd(Connection con, Storage storage) throws Exception {
		...
		pstmt.setString(3,
				DateUtil.formatDate(storage.getStoragedate(), "yyyy-MM-dd"));
		pstmt.setInt(4, storage.getStoragenumber());
		pstmt.setString(5, storage.getStoragenote());
		pstmt.setString(
				6,
				storage.getStorageunit() == null ? "" : storage
						.getStorageunit());
		pstmt.executeUpdate();
	...
	public int storageModify(Connection con, Storage storage) throws Exception {
		...
		// 修改库存信息
		String stockSql = "SELECT stockid, goodid, goodnumber, stocknote, outstockid, storageid FROM t_stock where storageid="
				+ storage.getId();
		PreparedStatement stockpstmt = con.prepareStatement(stockSql);
		...
		while (resultSet.next()) {
			...
			updatepstmt.setInt(
					2,
					goodnumber
							+ (storage.getStoragenumber() - storage
									.getStoragenumberold()));
