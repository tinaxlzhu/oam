...
public class OutstockDao {
	...
	public int outstockAdd(Connection con,OutStock outstock)throws Exception {
		...
		//修改库存信息
		String stockSql = "SELECT stockid, goodid, goodnumber, stocknote, outstockid, storageid FROM t_stock where goodid="+outstock.getGoodid();
	...
	public int outstockModify(Connection con,OutStock outstock)throws Exception{
		...
		//修改库存信息
		String stockSql = "SELECT stockid, goodid, goodnumber, stocknote, outstockid, storageid FROM t_stock where outstockid="+outstock.getOutstockid();
		PreparedStatement stockpstmt=con.prepareStatement(stockSql);
		...
		while(resultSet.next()){
			...
			updatepstmt.setInt(2, goodnumber-(outstock.getOutstocknumber()-outstock.getOutstocknumberold()));
